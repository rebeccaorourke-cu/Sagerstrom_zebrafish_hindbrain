---
title: "HB13hpf rep1 and rep2 neural refmapped integrated sample DIRECT-NET R Notebook"
...
for(group in 1:length(groups)){ 
  try({
    ...
    CREs_rhom <- generate_CRE_Gene_links(seurat_rhom_direct.net_human, 
                                         markers = rhom_markers_human[rhom_markers_human$group == rhom,])
    ...
    write.table(data.frame("TF"=rownames(Node_attribute_rhom),Node_attribute_rhom),
                file = paste0("results_neural_4logfc_rescued/",rhom,"_node_attribute.csv"),
                sep = ",", quote = F, row.names = FALSE)
    write.table(network_links_rhom,
                file = paste0("results_neural_4logfc_rescued/",rhom,"_network_links.csv"),
                sep = ",", quote = F, row.names = FALSE)
  })
...
# 8. Stats about Network analysis

```{r}
table(seurat_rhom_direct.net$function_type)
```

```{r}
table(seurat_rhom_direct.net_human$function_type)
```

```{r}
table(rhom_markers$group)
```

```{r}
table(rhom_markers_human$group)
```

```{r}
for(group in 1:length(da_peaks_list)){
  print(as.character(groups[[group]]))
  print(nrow(da_peaks_list[[group]]))
}
```
